...
def read_user(
    user_id: Annotated[str, Path(title="Id user yang ingin diambil")],
...
def write_user(user: WriteUserBase, db: Session = Depends(get_db)):
    ...
    return JSONResponse(
        content={"status": "User created succesfully", "user": user_dict}
    )


@router.put("/api/user/{user_id}", tags=["User"])
async def update_user(
    user_id: str, user_data: UpdateUserBase, db: Session = Depends(get_db)
):
    user = db.query(User).filter(User.id == user_id).first()
...
def delete_user(
    user_id: Annotated[str, Path(title="Id user yang ingin di delete")],
    db: Session = Depends(get_db),
):

...
def read_all_test_case_results():
    db = SessionLocal()
...
def read_test_case_result(test_case_result_id : int):
    db = SessionLocal()
...
def get_test_case_result(submission_id : int):
    db = SessionLocal()
...
def read_test_case_result_submission(submission_id : str):
    db = SessionLocal()
    db_test_case_result = db.query(TestCaseResult).filter(TestCaseResult.submission_id == submission_id).all()
    return db_test_case_result

@router.post("/api/test_case_result", tags=["Test Case Results"])
def write_test_case_result(new_test_case_result : WriteTestCaseResultBase):
    db = SessionLocal()
...
def delete_test_case_result(test_case_result_id : int):
    db = SessionLocal()
    db_test_case_result = db.query(TestCaseResult).filter(TestCaseResult.id == test_case_result_id).first()
